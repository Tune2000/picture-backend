...
public class FileManager {
    ...
    public UploadPictureResult uploadPicture(MultipartFile multipartFile, String uploadPathPrefix) {
        ...
        // 示例：/public/[loginUserId]/2000-01-01_[uuid].jpg
        String uploadPath = String.format("/%s/%s", uploadPathPrefix, uploadFilename);
        File file = null;
        try {
            // 创建临时文件
            file = File.createTempFile(uploadPath, null);
            // 将上传的文件内容传输到指定的 File 对象所代表的文件中
            multipartFile.transferTo(file);
            // 上传图片
     ...
     * @return
     */
    public String uploadLocalAvatar(MultipartFile file) {
        // 上传文件校验
        validPicture(file);
        ...
        // 本地根路径
        String uploadPath = "D:/avatar/uploadPath";
        ...
        // 生成访问图片的URL
        // /avatar/用户id/随机UUID.jpg (/avatar 是 ResourcesConfig 配置中的静态资源映射地址)
        return "/avatar/" + FilePath + "/" + uniqueFileName;
    }
